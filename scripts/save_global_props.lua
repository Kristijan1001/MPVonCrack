...
Documentation_ save_global_props.conf

Record global property changes, support restoration at next program startup. Data is saved in saved-props.json.
(The option --save-position-on-quit saves properties based on specific files; do not conflict with properties saved by --watch-later-options)

Available keybindings example (write in input.conf):
...
-- Reason: First version to add --watch-later-options option
local min_major = 0
local min_minor = 34
local min_patch = 0
	...
	for _, value1 in ipairs(table1) do
		for _, value2 in ipairs(table2) do
			if value1 == value2 then
				dup_opts = true
				mp.msg.warn("Project conflicts with --watch-later-options: " .. value1)
			end
		end
	end
end
...
if dup_opts and opt.dup_block then
	mp.msg.warn("Global property save/restore has been automatically disabled")
	return
end
...
local function save_data_file()
	if cleaned then
		mp.msg.verbose("Save function aborted due to property cleanup")
		return
	end
...
init()
mp.msg.info("Global property save/restore running in mode " .. opt.save_mode)
