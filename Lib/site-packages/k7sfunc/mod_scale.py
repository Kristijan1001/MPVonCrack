...
def CUGAN_NV(
	input : vs.VideoNode,
	lt_hd : bool = False,
	nr_lv : typing.Literal[-1, 0, 3, 4] = -1,
	sharp_lv : float = 1.0,
	gpu : typing.Literal[0, 1, 2] = 0,
	gpu_t : int = 2,
	st_eng : bool = False,
	...
	_validate_input_clip(func_name, input)
	_validate_bool(func_name, "lt_hd", lt_hd)
	_validate_literal(func_name, "nr_lv", nr_lv, [-1, 0, 3, 4])
	...
	plg_dir = os.path.dirname(core.trt.Version()["path"]).decode()
	mdl_fname = ["pro-no-denoise3x-up2x", "pro-conservative-up2x", "pro-denoise3x-up2x", "pro-denoise3x-up3x"][[-1, 0, 3, 4].index(nr_lv)]
	...
	cut2 = vsmlrt.CUGAN(clip=cut1, noise=actual_noise, scale=scale, alpha=sharp_lv, version=2, backend=vsmlrt.BackendV2.TRT(
		num_streams=gpu_t, force_fp16=True, output_format=1,
		workspace=None if ws_size < 128 else (ws_size if st_eng else ws_size * 2),
		use_cuda_graph=True, use_cublas=False, use_cudnn=False,
		static_shape=st_eng, min_shapes=[0, 0] if st_eng else [384, 384],
