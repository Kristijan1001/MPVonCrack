...
Documentation_ thumbfast.conf

New thumbnail engine adapted for multiple OSC-like scripts

Available keybindings example (write in input.conf):

 <KEY>   script-binding thumbfast/thumb_rerun    # Restart thumbnail acquisition (manually fix stuck thumbnails)
 <KEY>   script-binding thumbfast/thumb_toggle   # Toggle thumbnail preview
 <KEY>   script-message thumb_hwdec toggle       # Toggle thumbnail hardware decoding (replace {toggle} with specific API)
...
if incompat_check(mpv_ver_curr, min_major, min_minor, min_patch) then
    mp.msg.warn("Current mpv version (" .. (mpv_ver_curr or "unknown") .. ") is lower than " .. min_major .. "." .. min_minor .. "." .. min_patch .. ", thumbnail function terminated.")
    return
end
